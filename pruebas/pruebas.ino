  ...
  if (Serial.available()){
    ...
    if (dif == "M"){
      ...
      if (initial_position != pos){
        ...
        plate.step(initial_position_base, BACKWARD, MICROSTEP);
      ...
      fun(workstations, velocity, f_velocity);                          //velocidades
      int sep6 = orden.indexOf("/",sep5+1);
      String times = orden.substring(sep5+1,sep6);
      int f_times[workstations];
      fun(workstations, times, f_times);                                       //tiempos

      crane.step(f_alturas[workstations-1], FORWARD, MICROSTEP);     
      plate.step(f_posiciones[workstations-1], BACKWARD, MICROSTEP);
      //Serial.println("la grua da "+String(j)+" hacia atras");
      ...
      for (int j=0;j<ciclos; j++){
        ...
        for (int i=0; i<workstations; i++){
          plate.step(f_posiciones[i]-camino_base, FORWARD, MICROSTEP);
          //Serial.println("la grua da "+String(j)+" hacia atras");
          //Serial.println("la base da "+String(f_posiciones[i]-camino_base)+" hacia adelante");
          
          crane.setSpeed(f_velocity[i]);
          crane.step(f_alturas[i], BACKWARD, MICROSTEP);
          //Serial.println("la grua da "+String(j)+" hacia atras");
          //Serial.println("la grua da "+String(f_alturas[i])+" hacia adelante");
          delay(1000*f_times[i]);                 
          crane.step(f_alturas[i], FORWARD, MICROSTEP);
          //Serial.println("la grua da "+String(j)+" hacia atras");
